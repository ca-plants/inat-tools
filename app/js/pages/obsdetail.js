...
const DETAIL_COLS = {
    ...
    OBSERVER: new ColDef(
        ...
        ( obs ) => {
            return DOMUtils.createLinkElement(
                "https://www.inaturalist.org/people/" + INatObservation.getUserLogin( obs ),
    ...
    LOCATION: new ColDef(
        ...
        ( obs ) => {
            ...
            url.searchParams.set( "query", INatObservation.getCoordinatesString( obs ) );
            return DOMUtils.createLinkElement(
                url,
...
class ObsDetailUI extends UI {
    ...
    getObserverINatLink( login, count ) {
        ...
        const url = new URL( "https://www.inaturalist.org/observations?subview=grid" );
        url.searchParams.set( "lrank", this.#taxon_data.rank );
        return DOMUtils.createLinkElement( filter.getURL( url ), count, { target: "_blank" } );
    }
    ...
    async init( selArray ) {
        ...
        function addDisplayOption( value, label, onclick ) {
            const id = "disp-" + value;
            const div = DOMUtils.createElement( "div" );
            const rb = DOMUtils.createElement( "input", { type: "radio", id: id, value: value, name: "displayopt" } );
        ...
        const results = await DataRetriever.getObservationData( api, this.#f1, this.getProgressReporter() );
        if ( !results ) {
            return;
        }
        ...
        if ( projectID ) {
            const members = await DataRetriever.getProjectMembers( api, projectID, this.getProgressReporter() );
        ...
        const radios = DOMUtils.createElement( "div", { class: "displayoptions" } );
        addDisplayOption( "details", "Details", () => this.showDetails() );
        addDisplayOption( "geojson", "GeoJSON", () => this.showGeoJSON() );
        addDisplayOption( "usersumm", "Summary by Observer", () => this.showUserSumm() );

        const iNatDiv = DOMUtils.createElement( "div" );
        iNatDiv.appendChild( DOMUtils.createLinkElement( "", "View in iNaturalist", { target: "_blank", id: "viewininat" } ) );
    ...
    showDetails() {
        ...
        const cols = [ DETAIL_COLS.OBS_DATE, DETAIL_COLS.OBSERVER, DETAIL_COLS.LOCATION, DETAIL_COLS.COORDS ];
    ...
    showGeoJSON() {
        ...
        for ( const obs of this.#results.observations ) {
            ...
            const feature = { type: "Feature", properties: properties, geometry: { type: "Point", coordinates: INatObservation.getCoordinatesGeoJSON( obs ) } };
        ...
        urlGJ.hash = "data=data:application/json," + encodeURIComponent( JSON.stringify( geoJSON ) );
        const eBtnGeoJSONIO = DOMUtils.createLinkElement( urlGJ, "geojson.io", { target: "_blank" } );
    ...
    showUserSumm() {
        ...
        function getRow( userSumm, cols, ui ) {
            const tr = DOMUtils.createElement( "tr" );
            for ( const col of cols ) {
                ColDef.addColElement( tr, col.getValue( userSumm, ui ), col.getClass() );
            }
            return tr;
        }
        ...
        const cols = [ SUMMARY_COLS.OBSERVER, SUMMARY_COLS.NUM_OBS, SUMMARY_COLS.NUM_PUBLIC, SUMMARY_COLS.NUM_TRUSTED, SUMMARY_COLS.NUM_OBSCURED ];
        if ( this.#project_members ) {
        ...
        for ( const userSumm of Object.values( summary ).sort( ( a, b ) => b.count - a.count ) ) {
    ...
    #updateViewInINaturalistLink() {
        ...
        function getURL( ui ) {
            ...
            function showAll() {
                ...
                const filter = new SpeciesFilter( params );
                const url = filter.getURL( "https://www.inaturalist.org/observations" );
            ...
            switch ( selectedTypes.join( "," ) ) {
                ...
                default:
                    ...
                    if ( selectedTypes.length == 2 ) {
                        if ( ui.#results.countPublic === 0 || ui.#results.countTrusted === 0 || ui.#results.countObscured === 0 ) {
                            return showAll();
                        }
                    }
                    return "";

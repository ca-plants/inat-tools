...
class INatAPI {
    ...
    async #getAutoComplete( str, type, nameGen ) {
        const url = new URL( "https://api.inaturalist.org/v1/" + type + "/autocomplete" );
        url.searchParams.set( "q", str );
        const json = await this.getJSON( url );
        const results = {};
        for ( const result of json.results ) {
            results[ ( typeof nameGen === "string" ) ? result[ nameGen ] : nameGen( result ) ] = result.id;
        }
        return results;
    }
    ...
    async getPlaceData( placeID ) {
        return this.#getDataByID( placeID, "https://api.inaturalist.org/v1/places/" );
    }

    async getProjectData( projID ) {
        return this.#getDataByID( projID, "https://api.inaturalist.org/v1/projects/" );
    }

    async getTaxonData( taxonID ) {
        return this.#getDataByID( taxonID, "https://api.inaturalist.org/v1/taxa/" );
    }
    ...
    static getTaxonName( taxon ) {
        switch ( taxon.rank ) {
            ...
            case "variety": {
                ...
                parts.splice( 2, 0, { subspecies: "subsp.", variety: "var." }[ taxon.rank ] );
                return parts.join( " " );
            }
            default:
                return taxon.name;
    ...
    getTaxonFormName( taxon, addCommonName = true ) {
        let commonName = addCommonName ? taxon[ "preferred_common_name" ] : undefined;
        commonName = commonName ? ( " (" + commonName + ")" ) : "";
        if ( taxon.rank_level > 10 ) {
            const rank = taxon[ "rank" ];
            return rank[ 0 ].toUpperCase() + rank.substring( 1 ) + " " + taxon[ "name" ] + commonName;
        }

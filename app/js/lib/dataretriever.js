...
class DataRetriever {
    ...
    static async getSpeciesData( api, filtInclude, filtExclude, progressReporter ) {
        ...
        const exclude = await this.#retrieveSpeciesData( "exclusions", api, filtExclude, progressReporter );
    ...
    static async #retrievePagedData( baseURL, label, api, progressReporter ) {
        ...
        try {
            ...
            if ( totalResults > maxResults ) {
                await progressReporter.modalAlert( totalResults + " results found, maximum is " + maxResults );
                return;
            }
            ...
            if ( numPages > maxPages ) {
                await progressReporter.modalAlert( numPages + " pages found, maximum is " + maxPages );
                return;
            }
